...
from ..pytorch_dnc.core.accessors.dynamic_accessor import DynamicAccessor
...
def diff_fn(x, y):
        ...
        diff_kernel[grid](x, y, output, n_elements, BLOCK_SIZE=4)
        return output
...
class AutoEncoder(nn.Module):
    def __init__(self,num_features=None,num_target=None,param_dict=None,forward_dtype=None):
        super(AutoEncoder, self).__init__()
        self.forward_dtype  = forward_dtype
        self.encoder = nn.Sequential(
            nn.Linear(512, 256, dtype=self.forward_dtype),
            nn.ReLU(),
            nn.Linear(256, 128, dtype=self.forward_dtype),
            nn.ReLU(),
            nn.Linear(128, 64, dtype=self.forward_dtype),
            nn.ReLU(),
            nn.Linear(64, 32, dtype=self.forward_dtype),
            nn.ReLU(),
        )
        self.decoder = nn.Sequential(
            nn.Linear(32, 64, dtype=self.forward_dtype),
            nn.ReLU(),
            nn.Linear(64, 128, dtype=self.forward_dtype),
            nn.ReLU(),
            nn.Linear(128, 256, dtype=self.forward_dtype),
            nn.ReLU(),
            nn.Linear(256,512, dtype=self.forward_dtype),
            nn.ReLU()
        )
...
class TinyRecursiveReasoningModel_ACTV1(nn.Module):
    ...
    def forward(self, carry: TinyRecursiveReasoningModel_ACTV1Carry, batch: Dict[str, torch.Tensor]) -> Tuple[TinyRecursiveReasoningModel_ACTV1Carry, Dict[str, torch.Tensor]]:
        ...
        #print("new_inner_carry.z_H",new_inner_carry.z_H.shape)
        #print("next(self.agent.circuit.parameters()).is_cuda",next(self.agent.circuit.parameters()).is_cuda)
