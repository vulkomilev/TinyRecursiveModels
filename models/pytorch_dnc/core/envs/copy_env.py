...
class CopyEnv(Env):
    ...
    def _generate_sequence(self):
        ...
        arc_data = {"input": np.array([[7, 9], [4, 3]])/10.0,
         "output": np.array([[7, 9, 7, 9, 7, 9], [4, 3, 4, 3, 4, 3], [9, 7, 9, 7, 9, 7], [3, 4, 3, 4, 3, 4], [7, 9, 7, 9, 7, 9],
                    [4, 3, 4, 3, 4, 3]])/10.0}
        for batch_ind in range(self.batch_size):
            ...
            #data_target = np.random.randint(2, size=(num_words, self.len_word))
            data      = np.zeros(shape=(num_words, self.len_word))
            data_target = np.zeros(shape=(num_words, self.len_word))
            data[:arc_data["input"].shape[0],:arc_data["input"].shape[1]] = arc_data["input"]
            data_target[:arc_data["output"].shape[0], :arc_data["output"].shape[1]] = arc_data["output"]
            #data = np.random.randint(2, size=(num_words, self.len_word))
            ...
            self.exp_state1[1][batch_ind][num_words+2:num_words*2+2, :] = data_target
            # prepare mask   for this sample
            #self.exp_state1[2][batch_ind][num_words+2:num_words*2+2, :] = 1
            self.exp_state1[2][batch_ind][:, :] = 1
